...
class DetailViewController: UIViewController {
    ...
    @objc func addFavorite() {
        self.list = UserDefaults.standard.stringArray(forKey: "pokemon") ?? []
        
        if self.list.contains(name.text!) {
            let alert = UIAlertController(title: "You really like \(name.text ?? "this Pokemon"), don't you 😁", message: "This Pokemon is already on your Favorite list", preferredStyle: .alert)
            alert.addAction(UIAlertAction(title: "OK", style: .default, handler: nil))
            self.present(alert, animated: true, completion: nil)
            return
        } else {
            self.list.append(name.text!)
        }
        
        UserDefaults.standard.set(self.list, forKey: "pokemon")
    }
